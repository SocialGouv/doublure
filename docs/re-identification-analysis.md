...
| **Channel 2 — Bash** | Blocked, never pseudonymized | A command must reach the real cluster, so there is no return path. The hook refuses direct network egress and secret dumps before execution. Inherent; the definitive answer is D9 (deployment isolation). |
| **Channel 2 — remote MCP, WebFetch** | Pseudonymized under `task forward` | They speak HTTP, so the forward proxy terminates TLS and rewrites JSON-RPC bodies both ways, with the same vault. **Not the default path**, and not yet exercised by a real session — treat the guarantee as untested until it has been. |
